//! E2E Installation Test Runner (Serial Backend) for LevitateOS and AcornOS.
//!
//! Runs installation steps in QEMU using serial console for I/O.
//! This is the primary test runner for CI/CD and quick iteration.
//!
//! # STOP. READ. THEN ACT.
//!
...
use install_tests::{
    ...
    kill_stale_qemu_processes, require_preflight, steps_for_phase, CheckResult, CommandLog,
    Console, DistroContext, QemuBuilder, SerialExecutorExt, Step, StepResult, AVAILABLE_DISTROS,
};

#[derive(Parser)]
#[command(name = "install-tests")]
#[command(about = "E2E installation test runner for LevitateOS (serial backend)")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
    ...
    for (check_name, check_result) in checks {
        match check_result {
            ...
            CheckResult::Skip(reason) => {
                println!("    {} {}: {}", "[SKIP]".yellow(), check_name, reason);
            }
            CheckResult::Warning(reason) => {
                println!("    {} {}: {}", "[WARN]".yellow(), check_name, reason);
            }
        }
    }
}
...
fn run_tests(
    ...
    println!(
        "{}",
        format!("{} E2E Installation Tests (Serial Backend)", ctx.name()).bold()
    );
    println!();
    ...
    // This saves time by failing fast if initramfs/ISO is broken
    ...
    let pre_reboot_steps: Vec<_> = all_requested
        .iter()
        .filter(|s| s.num() <= 18)
        .map(|s| s.num())
        .collect();
    let post_reboot_steps: Vec<_> = all_requested
        .iter()
        .filter(|s| s.num() >= 19)
        .map(|s| s.num())
        .collect();
    ...
    if needs_pre_reboot {
        ...
        println!(
            "{}",
            "Installation complete, shutting down live ISO...".cyan()
        );
    ...
    if needs_post_reboot && all_passed {
        ...
        println!(
            "{}",
            format!("{} installed system booted!", ctx.name()).green()
        );
        ...
        if !boot_failures.is_empty() {
            ...
            println!(
                "{}",
                "!! Services failed during boot - will capture diagnostics".yellow()
            );
        ...
        if !boot_failures.is_empty() {
            ...
            println!(
                "{}",
                "CAPTURING DIAGNOSTICS FOR FAILED SERVICES".yellow().bold()
            );
            println!("{}", "=".repeat(60));
            println!();
                        ...
                        after_start.find(|c: char| c == ' ' || c == '-' || c == '.')
                    {
                        ...
                        for suffix in ["", ".service"] {
                            ...
                            let journal = console.exec(
                                &format!(
                                    "journalctl -u {} --no-pager -n 50 2>&1 || true",
                                    full_name
                                ),
                                Duration::from_secs(10),
                            )?;
            ...
            for service in [
                ...
                let status = console.exec(
                    &format!(
                        "systemctl status {} --no-pager 2>&1 | head -30 || true",
                        service
                    ),
                    Duration::from_secs(10),
                )?;
            ...
            let host_keys = console.exec(
                "ls -la /etc/ssh/ssh_host_* 2>&1 || echo 'NO HOST KEYS'",
                Duration::from_secs(5),
            )?;
    ...
    let phases_run: Vec<usize> = results
        .iter()
        .map(|r| match r.step_num {
            1..=2 => 1,
            3..=6 => 2,
            7..=10 => 3,
            11..=15 => 4,
    ...
    for phase in 1..=6 {
        ...
        let phase_results: Vec<_> = results
            .iter()
            .filter(|r| {
    ...
    if all_passed {
        ...
        if total_skips > 0 || total_warnings > 0 {
            println!(
                "  - {} Some features were not tested or have warnings",
                "[WARN]".yellow()
            );
        } else {
        ...
        if total_skips > 0 || total_warnings > 0 {
            println!(
                "{}",
                "This rootfs passed but has gaps. Review skips/warnings above."
                    .yellow()
                    .bold()
            );
        } else {
            println!(
                "{}",
                "This rootfs is ready for daily driver use.".green().bold()
            );
        }
    } else {
        println!("{}", "=".repeat(60));
        ...
        if total_skips > 0 {
            println!("  {} {} checks skipped", "[SKIP]".yellow(), total_skips);
        }
        if total_warnings > 0 {
            println!("  {} {} warnings", "[WARN]".yellow(), total_warnings);
        }
        println!("{}", "=".repeat(60));
        println!();
